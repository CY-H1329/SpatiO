...
def _load_role_prompts() -> Dict[str, str]:
    """
    Source de vérité: roles/<SPATIO_ROLE_SET>/prompts/*.txt si disponible.
    Fallback: prompts codés en dur (compat rétro).
    """
    try:
        rs = load_role_set()
        return {r.role_id: read_role_prompt(r.role_id, set_id=rs.set_id) for r in rs.roles}
    except Exception:
        return {
...
_ROLE_PROMPTS = _load_role_prompts()
...
def build_role_prompt(
    ...
    # Some role templates embed "{tool_output}" directly.
    # Always provide it (empty string if None) to avoid KeyError.
    prompt = template.format(query=query, tool_output=(tool_output or ""))

    # Ajoute tool output si le template le supporte (templates new_roles_v1 l'incluent aussi)
    if tool_output and ("{tool_output}" in template or role in ("explicit_3d_representation", "scene_graph_construction")):
