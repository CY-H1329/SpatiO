...
Usage:
  python run_mmsi.py --max_samples 500 --test_only
  python run_mmsi.py --max_samples 500 --test_only --device_map="1,3,4,5,6"
  MODEL_ROOT=/path/to/project python run_mmsi.py --max_samples 500 --test_only
"""
import argparse
import json
import logging
...
# Add SpatiO root
SPATIO_ROOT = Path(__file__).resolve().parent
sys.path.insert(0, str(SPATIO_ROOT))

# Add host project for models (when MODEL_ROOT is set)
MODEL_ROOT = os.environ.get("MODEL_ROOT", str(SPATIO_ROOT.parent))
...
from benchmarks import load_benchmark, get_benchmark_image, get_benchmark_prompt, get_benchmark_answer, get_benchmark_category
